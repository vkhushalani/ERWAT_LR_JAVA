...
public class Call_a_GET_API {
	public JSONArray getUserDetails(String userId){
		...
		String resourceUrl = "https://api2preview.sapsf.eu/odata/v2/User?$filter=userId eq '"+userId+"'&$format=json&$select=userId,username,firstName,lastName";
...
//		logger.error(request.getHeaders().toString());
	...
	public JSONArray searchUser(String searchstr){
		RestTemplate restTemplate = new RestTemplate();
		String resourceUrl = "https://api2preview.sapsf.eu/odata/v2/User?$format=json&$top=10&$filter=substringof(toupper('"+searchstr+"'),toupper(firstName)) or substringof(toupper('"+searchstr+"'),toupper(lastName))&$select=userId,username,defaultFullName";
		HttpHeaders headers = new HttpHeaders();
		String plainCreds = "nga_anujk@erandwaterT1:nga@2018";
		byte[] plainCredsBytes = plainCreds.getBytes();
		byte[] base64CredsBytes = Base64.encodeBase64(plainCredsBytes);
